...
export default class TransactionInfo extends React.PureComponent<{},{result:any[]}> {
  ...
  render() {
    return (
      <>
        <AddTransactionForm />
        <TransactionHistory result={this.state.result} />
      </>
    );
  }
}

const USER_DETAILS_FIELDS=["customer_id",  "first_name",  "last_name",  "email",  "gender", "country","street","city", "phone"];
const CREDIT_FIELDS=["total_price","currency","cerdit_card_type","cerdit_card_number"]
...
function TransactionHistory(props: any) {
  ...
  return <div>
    {result?.map((transaction:billingSystem.TransactionType) => <div key={transaction._id}>{transaction.first_name}</div>)}
    </div>;
}
